#include <JY901S.h>

char cmd[10] = {'a', 'm', 'e', 'u', 'U', 'B', 'b', 'R', 'r', 'C'};
static volatile char s_cDataUpdate = 0, s_cCmd = 0xff;
const uint32_t c_uiBaud[8] = {0, 4800, 9600, 19200, 38400, 57600, 115200, 230400};

static void CmdProcess(void);
static void AutoScanSensor(void);
static void SensorUartSend(uint8_t *p_data, uint32_t uiSize);
static void CopeSensorData(uint32_t uiReg, uint32_t uiRegNum);
static int32_t IICreadBytes(uint8_t dev, uint8_t reg, uint8_t *data, uint32_t length);
static int32_t IICwriteBytes(uint8_t dev, uint8_t reg, uint8_t *data, uint32_t length);
static void Delayms(uint16_t ucMs);

void JY901S::JY901S_Init()
{
    Wire.begin(3, 2);                                // 初始到wire库，并且加入IIC网络
    Wire.setClock(40000);                            // 设置IIC的速度为40000
    WitInit(WIT_PROTOCOL_I2C, 0x50);                 // 初始化WitI2c
    WitI2cFuncRegister(IICwriteBytes, IICreadBytes); // 注册IIC读写函数
    WitRegisterCallBack(CopeSensorData);             // 注册IIC读写回调函数
    WitDelayMsRegister(Delayms);                     // 注册延时函数
    AutoScanSensor();                                // 自动扫描传感器
}

int i;
float fAcc[3], fGyro[3], fAngle[3];
float JY901S::GetAccX()
{
    WitReadReg(AX, 12); // 读取寄存器
    delay(5000);
    CmdProcess(); // 处理命令

    if (s_cDataUpdate)
    {
        for (uint8_t i = 0; i < 3; i++)
        {
            fAcc[i] = sReg[AX + i] / 32768.0f * 16.0f;
            fGyro[i] = sReg[GX + i] / 32768.0f * 2000.0f;
            fAngle[i] = sReg[Roll + i] / 32768.0f * 180.0f;
        }
        if (s_cDataUpdate & ACC_UPDATE)
        {
            Serial.println("AccX:");//(pitch)
            Serial.println(fAngle[0], 3);
            Serial.println("AccY:");//(roll)
            Serial.println(fAngle[1], 3);
            Serial.println("AccZ:");//(yaw)
            Serial.println(fAngle[2], 3);
            Serial.println("");
            s_cDataUpdate &= ~ACC_UPDATE;
            return fAngle[0];
        }
        s_cDataUpdate = 0;
    }
    return fAcc[0];
}
void CopeCmdData(unsigned char ucData)
{
    static unsigned char s_ucData[50], s_ucRxCnt = 0;

    s_ucData[s_ucRxCnt++] = ucData;
    if (s_ucRxCnt < 3)
        return; // Less than three data returned
    if (s_ucRxCnt >= 50)
        s_ucRxCnt = 0;
    if (s_ucRxCnt >= 3)
    {
        if ((s_ucData[1] == '\r') && (s_ucData[2] == '\n'))
        {
            s_cCmd = s_ucData[0];
            memset(s_ucData, 0, 50);
            s_ucRxCnt = 0;
        }
        else
        {
            s_ucData[0] = s_ucData[1];
            s_ucData[1] = s_ucData[2];
            s_ucRxCnt = 2;
        }
    }
}

static int32_t IICreadBytes(uint8_t dev, uint8_t reg, uint8_t *data, uint32_t length)
{
    int val;
    Wire.beginTransmission(dev);
    Wire.write(reg);
    Wire.endTransmission(false); // endTransmission but keep the connection active

    val = Wire.requestFrom(dev, length); // Ask for bytes, once done, bus is released by default

    if (val == 0)
        return 0;
    for (int x = 0; x < length; x++)
        data[x] = Wire.read();

    return 1;
}

static int32_t IICwriteBytes(uint8_t dev, uint8_t reg, uint8_t *data, uint32_t length)
{
    Wire.beginTransmission(dev);
    Wire.write(reg);
    Wire.write(data, length);
    Wire.endTransmission(); // Stop transmitting

    return 1;
}

static void ShowHelp(void)
{
    Serial.println("\r\n************************	 WIT_SDK_DEMO	************************");
    Serial.println("\r\n************************          HELP           ************************\r\n");
    Serial.println("UART SEND:a\\r\\n   Acceleration calibration.\r\n");
    Serial.println("UART SEND:m\\r\\n   Magnetic field calibration,After calibration send:   e\\r\\n   to indicate the end\r\n");
    Serial.println("UART SEND:U\\r\\n   Bandwidth increase.\r\n");
    Serial.println("UART SEND:u\\r\\n   Bandwidth reduction.\r\n");
    Serial.println("UART SEND:B\\r\\n   Baud rate increased to 115200.\r\n");
    Serial.println("UART SEND:b\\r\\n   Baud rate reduction to 9600.\r\n");
    Serial.println("UART SEND:h\\r\\n   help.\r\n");
    Serial.println("******************************************************************************\r\n");
}

static void CmdProcess(void)
{
    switch (s_cCmd)
    {
    case 'a':
        if (WitStartAccCali() != WIT_HAL_OK)
            Serial.println("\r\nSet AccCali Error\r\n");
        break;
    case 'm':
        if (WitStartMagCali() != WIT_HAL_OK)
            Serial.println("\r\nSet MagCali Error\r\n");
        break;
    case 'e':
        if (WitStopMagCali() != WIT_HAL_OK)
            Serial.println("\r\nSet MagCali Error\r\n");
        break;
    case 'u':
        if (WitSetBandwidth(BANDWIDTH_5HZ) != WIT_HAL_OK)
            Serial.println("\r\nSet Bandwidth Error\r\n");
        break;
    case 'U':
        if (WitSetBandwidth(BANDWIDTH_256HZ) != WIT_HAL_OK)
            Serial.println("\r\nSet Bandwidth Error\r\n");
        break;
    case 'B':
        if (WitSetUartBaud(WIT_BAUD_115200) != WIT_HAL_OK)
            Serial.println("\r\nSet Baud Error\r\n");
        else
            Serial.println(" 115200 Baud rate modified successfully\r\n");
        break;
    case 'b':
        if (WitSetUartBaud(WIT_BAUD_9600) != WIT_HAL_OK)
            Serial.println("\r\nSet Baud Error\r\n");
        else
            Serial.println(" 9600 Baud rate modified successfully\r\n");
        break;
    case 'h':
        ShowHelp();
        break;
    default:
        return;
    }
    s_cCmd = 0xff;
}

static void CopeSensorData(uint32_t uiReg, uint32_t uiRegNum)
{
    int i;
    for (i = 0; i < uiRegNum; i++)
    {
        switch (uiReg)
        {
        case AZ:
            s_cDataUpdate |= ACC_UPDATE;
            break;
        case GZ:
            s_cDataUpdate |= GYRO_UPDATE;
            break;
        case HZ:
            s_cDataUpdate |= MAG_UPDATE;
            break;
        case Yaw:
            s_cDataUpdate |= ANGLE_UPDATE;
            break;
        default:
            s_cDataUpdate |= READ_UPDATE;
            break;
        }
        uiReg++;
    }
}

static void Delayms(uint16_t ucMs)
{
    delay(ucMs);
}

static void AutoScanSensor(void)
{
    int i, iRetry;

    for (i = 0; i < 0x7F; i++)
    {
        WitInit(WIT_PROTOCOL_I2C, i);
        iRetry = 2;
        do
        {
            s_cDataUpdate = 0;
            WitReadReg(AX, 3);
            delay(5);
            if (s_cDataUpdate != 0)
            {
                Serial.println("find 0x");
                Serial.println(i, HEX);
                Serial.println(" addr sensor\r\n");
                ShowHelp();
                return;
            }
            iRetry--;
        } while (iRetry);
    }
    Serial.println("can not find sensor\r\n");
    Serial.println("please check your connection\r\n");
}

void GetAcc_task(void *pvParameters)
{
    JY901S *gyroscope = (JY901S *)pvParameters;

    gyroscope->JY901S_Init();
    while (1)
    {
        gyroscope->GetAccX();
        delay(1000);
    }

    vTaskDelete(NULL);
}